...
fn handle_full_data_option(src: &mut SacnSource, split_input: Vec<&str>) -> ExampleResult<bool> {
...
fn handle_all_data_option(src: &mut SacnSource, split_input: Vec<&str>) -> ExampleResult<bool> {
...
///
/// split_input[3] is expected to be the priority to send the data with.
fn handle_data_over_time_option(src: &mut SacnSource, split_input: Vec<&str>) -> ExampleResult<bool> {
    if split_input.len() < 4 {
        Err(std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            "Insufficient parts for data line ( < 4 )",
...
fn handle_test_preset_option(src: &mut SacnSource, split_input: Vec<&str>) -> ExampleResult<bool> {
    if split_input.len() < 3 {
        Err(std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
...
fn run_test_moving_channel_preset(src: &mut SacnSource, universe: u16) -> ExampleResult<()> {
    let start_time = Instant::now();
    ...
    while start_time.elapsed() < TEST_PRESET_DURATION {
        ...
        for i in 1..data.len() {
            let x: f64 = ((start_time.elapsed().as_millis() as f64) + (i as f64) * MOVING_CHANNEL_TEST_WAVE_OFFSET)
...
///
/// universe: The universe to send data on in the test.
fn run_test_rapid_changes_preset(src: &mut SacnSource, universe: u16) -> ExampleResult<()> {
    let start_time = Instant::now();
...
fn run_test_high_data_rate(src: &mut SacnSource, start_universe: u16, universe_count: u16) -> ExampleResult<()> {
    let start_time = Instant::now();
...
fn run_acceptance_test_demo(src: &mut SacnSource) -> ExampleResult<()> {
...
fn gen_acceptance_test_step_1(buf: &mut [u8; UNIVERSE_CHANNEL_CAPACITY * 2]) {
    ...
    gen_acceptance_test_step_1_backlight_state(
        &mut buf[ACCEPT_TEST_BACKLIGHT_ADDR_1..ACCEPT_TEST_BACKLIGHT_ADDR_1 + ACCEPT_TEST_BACKLIGHT_CH_COUNT],
    );
    gen_acceptance_test_step_1_backlight_state(
        &mut buf[ACCEPT_TEST_BACKLIGHT_ADDR_2..ACCEPT_TEST_BACKLIGHT_ADDR_2 + ACCEPT_TEST_BACKLIGHT_CH_COUNT],
    );
    gen_acceptance_test_step_1_backlight_state(
        &mut buf[ACCEPT_TEST_BACKLIGHT_ADDR_3..ACCEPT_TEST_BACKLIGHT_ADDR_3 + ACCEPT_TEST_BACKLIGHT_CH_COUNT],
    );
    gen_acceptance_test_step_1_backlight_state(
        &mut buf[ACCEPT_TEST_BACKLIGHT_ADDR_4..ACCEPT_TEST_BACKLIGHT_ADDR_4 + ACCEPT_TEST_BACKLIGHT_CH_COUNT],
    );
    gen_acceptance_test_step_1_backlight_state(
        &mut buf[ACCEPT_TEST_BACKLIGHT_ADDR_5..ACCEPT_TEST_BACKLIGHT_ADDR_5 + ACCEPT_TEST_BACKLIGHT_CH_COUNT],
    );
    gen_acceptance_test_step_1_backlight_state(
        &mut buf[ACCEPT_TEST_BACKLIGHT_ADDR_6..ACCEPT_TEST_BACKLIGHT_ADDR_6 + ACCEPT_TEST_BACKLIGHT_CH_COUNT],
    );
    gen_acceptance_test_step_1_backlight_state(
        &mut buf[ACCEPT_TEST_BACKLIGHT_ADDR_7..ACCEPT_TEST_BACKLIGHT_ADDR_7 + ACCEPT_TEST_BACKLIGHT_CH_COUNT],
    );
    gen_acceptance_test_step_1_backlight_state(
        &mut buf[ACCEPT_TEST_BACKLIGHT_ADDR_8..ACCEPT_TEST_BACKLIGHT_ADDR_8 + ACCEPT_TEST_BACKLIGHT_CH_COUNT],
    );

    // Frontlights.
    gen_acceptance_test_step_1_frontlight_state(
        &mut buf[ACCEPT_TEST_FRONTLIGHT_ADDR_1..ACCEPT_TEST_FRONTLIGHT_ADDR_1 + ACCEPT_TEST_FRONTLIGHT_CH_COUNT],
    );
    gen_acceptance_test_step_1_frontlight_state(
        &mut buf[ACCEPT_TEST_FRONTLIGHT_ADDR_2..ACCEPT_TEST_FRONTLIGHT_ADDR_2 + ACCEPT_TEST_FRONTLIGHT_CH_COUNT],
    );
    gen_acceptance_test_step_1_frontlight_state(
        &mut buf[ACCEPT_TEST_FRONTLIGHT_ADDR_3..ACCEPT_TEST_FRONTLIGHT_ADDR_3 + ACCEPT_TEST_FRONTLIGHT_CH_COUNT],
    );
}
...
fn handle_input(src: &mut SacnSource) -> ExampleResult<bool> {
                ...
                ACTION_PREVIEW_OPTION => {
                    ...
                    match val {
                        ...
                        Err(_e) => {
                            Err(std::io::Error::new(
                                std::io::ErrorKind::InvalidInput,
                                "Preview flag option not 'true'/'false' or otherwise parsable as boolean",
                            ))?;
                        }
                    }
                    Ok(true)
