...
const TEST_NETWORK_INTERFACE_IPV6: [&str; 3] = [
    "2a02:c7f:d20a:c600:a502:2dae:7716:601b",
    "2a02:c7f:d20a:c600:a502:2dae:7716:601c",
    "2a02:c7f:d20a:c600:a502:2dae:7716:601d",
];
...
mod sacn_ipv6_multicast_test {
    ...
    use sacn::{
        error::Error,
        packet::{
            universe_to_ipv4_multicast_addr, universe_to_ipv6_multicast_addr, ACN_SDT_MULTICAST_PORT, E131_DISCOVERY_UNIVERSE,
            E131_TERMINATE_STREAM_PACKET_COUNT, UNIVERSE_CHANNEL_CAPACITY,
        },
    ...
    fn test_send_recv_partial_capacity_universe_multicast_ipv6() {
        ...
        let rcv_thread = thread::spawn(move || {
            let mut dmx_recv = SacnReceiver::with_ip(
                SocketAddr::new(IpAddr::V6(TEST_NETWORK_INTERFACE_IPV6[0].parse().unwrap()), ACN_SDT_MULTICAST_PORT),
                None,
            )
            .unwrap();
        ...
        let ip: SocketAddr = SocketAddr::new(
            IpAddr::V6(TEST_NETWORK_INTERFACE_IPV6[1].parse().unwrap()),
    ...
    fn test_across_alternative_startcode_universe_multicast_ipv6() {
        ...
        src.send(
            &UNIVERSES,
            &TEST_DATA_MULTIPLE_ALTERNATIVE_STARTCODE_UNIVERSE,
            Some(priority),
            None,
            Some(UNIVERSES[0]),
        )
    ...
    fn test_send_across_universe_multiple_receivers_sync_multicast_ipv6() {
        ...
        let rcv_thread2 = thread::spawn(move || {
            let mut dmx_recv = SacnReceiver::with_ip(
                SocketAddr::new(IpAddr::V6(TEST_NETWORK_INTERFACE_IPV6[1].parse().unwrap()), ACN_SDT_MULTICAST_PORT),
                None,
            )
            .unwrap();
        ...
        let mut result = [received_result1[0].clone(), received_result2[0].clone()];
        result.sort_unstable(); // Ordering of received data is undefined, to make it easier to check sort first.
    ...
    fn test_three_senders_three_recv_multicast_ipv6() {
        ...
        assert!(
            RCV_THREADS <= TEST_NETWORK_INTERFACE_IPV6.len(),
        ...
        for i in 0..RCV_THREADS {
            ...
            rcv_threads.push(thread::spawn(move || {
                ...
                let mut dmx_recv = SacnReceiver::with_ip(
                    SocketAddr::new(IpAddr::V6(TEST_NETWORK_INTERFACE_IPV6[i].parse().unwrap()), ACN_SDT_MULTICAST_PORT),
                    None,
                )
                .unwrap();
                ...
                // SacnResults of each receive are sent back, this allows checking that each receiver was an expected universe, all universes were received and there were no errors.
                tx.send(res).unwrap();
            }));
    ...
    fn test_universe_discovery_one_universe_one_source_ipv6() {
        ...
        const SOURCE_NAMES: [&str; 1] = ["Source 1"];
    ...
    fn test_ansi_e131_appendix_b_runthrough_ipv6() {
        ...
        let mut dmx_recv = SacnReceiver::with_ip(
            SocketAddr::new(TEST_NETWORK_INTERFACE_IPV6[1].parse().unwrap(), ACN_SDT_MULTICAST_PORT),
            None,
        )
        .unwrap();
    ...
    fn test_ip_equivalence() {
        ...
        let mut ipv4_source = SacnSource::with_cid_ip(&source_name.clone(), Uuid::from_slice(&CID).unwrap(), ipv4).unwrap();
        ...
        ipv4_recv
            .bind(&SocketAddr::new(IpAddr::V4(Ipv4Addr::UNSPECIFIED), ACN_SDT_MULTICAST_PORT).into())
            .unwrap();
        ipv4_recv
            .join_multicast_v4(ipv4_multicast_addr.as_socket_ipv4().unwrap().ip(), &Ipv4Addr::UNSPECIFIED)
            .unwrap();
        ipv4_recv
            .join_multicast_v4(ipv4_discovery_multicast_addr.as_socket_ipv4().unwrap().ip(), &Ipv4Addr::UNSPECIFIED)
            .unwrap();
        ...
        let mut ipv6_source = SacnSource::with_cid_ip(&source_name.clone(), Uuid::from_slice(&CID).unwrap(), ipv6).unwrap();
        ...
        ipv6_recv
            .bind(&SocketAddr::new(IpAddr::V6(Ipv6Addr::UNSPECIFIED), ACN_SDT_MULTICAST_PORT).into())
            .unwrap();
        ipv6_recv
            .join_multicast_v6(ipv6_multicast_addr.as_socket_ipv6().unwrap().ip(), 0)
            .unwrap();
        ipv6_recv
            .join_multicast_v6(ipv6_discovery_multicast_addr.as_socket_ipv6().unwrap().ip(), 0)
            .unwrap();
        ...
        assert_eq!(
            ipv4_recv_buf[..ipv4_len],
            ipv6_recv_buf[..ipv6_len],
            "IPv4 and IPv6 data packets aren't identical"
        );
        ...
        assert_eq!(
            ipv4_recv_buf[..ipv4_len],
            ipv6_recv_buf[..ipv6_len],
            "IPv4 and IPv6 sync packets aren't identical"
        );
        ...
        assert_eq!(
            ipv4_recv_buf[..ipv4_len],
            ipv6_recv_buf[..ipv6_len],
            "IPv4 and IPv6 discovery packets aren't identical"
        );
        ...
        for _ in 0..E131_TERMINATE_STREAM_PACKET_COUNT {
            ...
            assert_eq!(
                ipv4_recv_buf[..ipv4_len],
                ipv6_recv_buf[..ipv6_len],
                "IPv4 and IPv6 termination packets aren't identical"
            );
        }
    }
}
...
mod sacn_ipv6_unicast_test {
    ...
    use sacn::{
        packet::{ACN_SDT_MULTICAST_PORT, UNIVERSE_CHANNEL_CAPACITY},
        receive::{DMXData, SacnReceiver},
        source::SacnSource,
        SacnResult,
    };
