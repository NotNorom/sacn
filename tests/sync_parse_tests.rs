...
pub mod sync_parse_tests {
    ...
    use sacn::{error::Error, packet::*, sacn_parse_pack_error};
    use uuid::Uuid;
    ...
    const TEST_SYNCHRONIZATION_PACKET: &[u8] = &[
        // Root Layer
        // Preamble Size
        0x00, 0x10, // Post-amble Size
        0x00, 0x00, // ACN Packet Identifier
        0x41, 0x53, 0x43, 0x2d, 0x45, 0x31, 0x2e, 0x31, 0x37, 0x00, 0x00, 0x00, // Flags and Length Protocol
        0x70, 0x21, // Vector
        0x00, 0x00, 0x00, 0x08, // CID
        0xef, 0x07, 0xc8, 0xdd, 0x00, 0x64, 0x44, 0x01, 0xa3, 0xa2, 0x45, 0x9e, 0xf8, 0xe6, 0x14,
        0x3e, // Synchronization Packet Framing Layer
        // Flags and Length
        0x70, 0x0b, // Vector
        0x00, 0x00, 0x00, 0x01,
        // Sequence Number - Specifies a value of 367 which doesn't fit within a unsigned 8-bit byte, therefore used 367 % 0xff = 0x70
        0x70, // Synchronization Address = 7962
        0x1F, 0x1A, // Reserved
        0, 0,
    ];
    ...
    const TEST_SYNCHRONIZATION_PACKET_TOO_HIGH_SYNC_ADDRESS: &[u8] = &[
        ...
        0x00, 0x00, 0x00, 0x01,
        // Sequence Number - Specifies a value of 367 which doesn't fit within a unsigned 8-bit byte, therefore used 367 % 0xff = 0x70
        0x70, // Synchronization Address = 64000 = E131_MAX_MULTICAST_UNIVERSE + 1
        0xFA, 0x00, // Reserved
        0, 0,
    ];
    ...
    const TEST_SYNCHRONIZATION_PACKET_TOO_LOW_SYNC_ADDRESS: &[u8] = &[
        ...
        0x00, 0x00, 0x00, 0x01,
        // Sequence Number - Specifies a value of 367 which doesn't fit within a unsigned 8-bit byte, therefore used 367 % 0xff = 0x70
        0x70, // Synchronization Address = 0 which is below the expected range
        0x00, 0x00, // Reserved
        0, 0,
    ];
    ...
    fn test_sync_packet_framing_layer_length_too_long_parse() {
        match AcnRootLayerProtocol::parse(
            ...
            Err(e) => match e {
                Error::SacnParsePackError(
                    sacn_parse_pack_error::ParsePackError::ParseInsufficientData(_),
                ) => {
                    assert!(true, "Expected error returned");
                }
                _ => {
    ...
    fn test_sync_packet_framing_layer_length_too_short_parse() {
        match AcnRootLayerProtocol::parse(
            ...
            Err(e) => match e {
                Error::SacnParsePackError(
                    sacn_parse_pack_error::ParsePackError::PduInvalidLength(_),
                ) => {
                    assert!(true, "Expected error returned");
                }
                _ => {
